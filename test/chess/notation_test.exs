defmodule Chess.NotationTest do
  ...
  describe "parse" do
    test "should parse simple pawn move (e4)" do
      ...
      assert {:ok, move} = Notation.parse(board, nil, :white, "e4")
    ...
    test "should parse pawn capture (dxe5)" do
      ...
      assert {:ok, move} = Notation.parse(board, nil, :white, "dxe5")
    ...
    test "should parse pawn capture from file (axb3)" do
      ...
      assert {:ok, move} = Notation.parse(board, nil, :white, "axb3")
    ...
    test "should parse piece move (Nf3)" do
      board = Board.from_shorthand!("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR")
      assert {:ok, move} = Notation.parse(board, nil, :white, "Nf3")
    ...
    test "should resolve ambiguity using file (Nbd7)" do
      ...
      # Need a black piece at d7 or just an empty square (valid for knight)

      assert {:ok, move} = Notation.parse(board, nil, :white, "Nbd7")
    ...
    test "should parse promotion (e8=Q)" do
      ...
      assert {:ok, move} = Notation.parse(board, nil, :white, "e8=Q")
